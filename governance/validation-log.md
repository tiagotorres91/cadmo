...
| 2026-07-14 | tooling @ `v0.4.2` | round-6 external audit (collaborator's independent GPT agent, read-only, on a clean Windows machine) + maintainer triage | 🔧 changes | 7 confirmed defects, all fixed with regression locks: `--verify` was verdict-blind (a logged *changes requested* verified as the approval — the Governed guarantee itself); spec-drift escapes via uncommitted work, renames (destination-only diff) and `master` default branches; `--stamp` leaked lone LF/CR into CRLF specs; unvalidated dates corrupted the log table; npm/plugin shipped MIT-only metadata while distributing CC BY templates (now `(MIT AND CC-BY-4.0)` + LICENSE in the tarball). Log now records the full sha256 (was 12 chars) and the scaffolder injects sync markers into copied mechanisms — the method's own return-lane rule, finally applied to its own product |
