...
**5. The path walker** — a fresh-context agent **operates** your project from a role you never occupy — the newcomer following the getting-started literally, the collaborator without permissions, the client reading the governance docs, the continuity dev standing up the environment from the doc alone — following *only what is written*. The structural reason: each role only sees the holes on its own side, and whoever holds all the privileges never hits the unprivileged paths — solo dogfooding validates the mechanism, never the other roles' experience (the process-level twin of "the pair shares its biases"). The scar behind it: this repo's first external collaborator hit, in one day of friction, day-0 gaps that four adversarial review rounds had never seen — and an agent walking the newcomer's literal path then reproduced the find cheaply.
